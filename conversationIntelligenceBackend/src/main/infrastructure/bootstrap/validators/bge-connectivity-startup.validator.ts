...
export class BgeConnectivityStartupValidator implements StartupValidator {
  ...
  public async validate(): Promise<void> {
    ...
    if (provider !== 'bge') {
      throw new Error(`Dependency BGE failed: unsupported embedding provider "${embeddingConfig.provider}".`);
    }
    ...
    try {
      ...
      throw new Error(`Dependency BGE failed at ${endpoint}: ${String(error)}`);
    }

    if (!response.ok) {
      throw new Error(
        `Dependency BGE failed at ${endpoint}: /embed returned ${response.status} ${response.statusText}.`
      );
    }
    ...
    if (!Array.isArray(payload.vector) || payload.vector.some((value) => typeof value !== 'number')) {
      throw new Error('Dependency BGE failed: invalid response payload. Expected { "vector": number[] }.');
    }
  }
}
