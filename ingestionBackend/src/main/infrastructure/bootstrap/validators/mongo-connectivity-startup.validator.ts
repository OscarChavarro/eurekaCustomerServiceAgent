...
export class MongoConnectivityStartupValidator implements StartupValidator {
  ...
  public async validate(): Promise<void> {
    ...
    try {
      ...
      await this.ensureEmbeddingsIndexes(database);
    } catch (error) {
      throw new Error(
  ...
  private async ensureEmbeddingsIndexes(database: Db): Promise<void> {
    const embeddingsCollection = database.collection('embeddings');
    await embeddingsCollection.createIndex(
      { conversationId: 1 },
      { name: 'idx_embeddings_conversationId' }
    );
    await embeddingsCollection.createIndex(
      { chunkId: 1 },
      { name: 'idx_embeddings_chunkId' }
    );
  }
}
