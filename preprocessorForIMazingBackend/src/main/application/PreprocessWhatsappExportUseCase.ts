...
import { ContactsBackendPort, CsvParserPort, DisabledContactsPort, FileSystemPort, LoggerPort } from './ports';
...
export class PreprocessWhatsappExportUseCase {
  ...
  constructor(
    ...
    @Inject(TOKENS.CsvParserPort)
    csvParser: CsvParserPort,
    @Inject(TOKENS.ContactsBackendPort)
    contactsBackend: ContactsBackendPort,
    @Inject(TOKENS.LoggerPort)
    logger: LoggerPort,
    @Inject(TOKENS.DisabledContactsPort)
    private readonly disabledContactsPort: DisabledContactsPort
  ) {
  ...
  async execute(rootFolderPath: string): Promise<void> {
    ...
    const disabledContactPatterns = await this.disabledContactsPort.load();

    await this.ensureAuxiliaryCsvFoldersUseCase.execute(rootFolderPath);
    await this.moveDisabledCsvFilesUseCase.execute(rootFolderPath, csvFolderPath, disabledContactPatterns);
