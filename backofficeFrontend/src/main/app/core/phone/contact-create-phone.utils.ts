export type ContactCreatePhoneNormalizationResult = {
  normalizedPhone: string | null;
  isValid: boolean;
};

const KNOWN_COUNTRY_CALLING_CODES = new Set<string>([
...
export function normalizePhoneForContactCreate(
  rawPhone: string | null | undefined
): ContactCreatePhoneNormalizationResult {
  if (typeof rawPhone !== 'string') {
    return { normalizedPhone: null, isValid: false };
  }
  ...
  if (compact.startsWith('+')) {
    ...
    if (!digits || !startsWithKnownCountryCallingCode(digits)) {
      return { normalizedPhone: null, isValid: false };
    }

    return { normalizedPhone: `+${digits}`, isValid: true };
  }
  ...
  if (startsWithKnownCountryCallingCode(digits)) {
    return { normalizedPhone: `+${digits}`, isValid: true };
  }

  // Local Spain mobile fallback (example: 698940748 -> +34698940748)
  if (/^6\d{8}$/.test(digits)) {
    return { normalizedPhone: `+34${digits}`, isValid: true };
  }

  return { normalizedPhone: null, isValid: false };
}

function startsWithKnownCountryCallingCode(digits: string): boolean {
  const maxPrefixLength = Math.min(3, digits.length);

  for (let length = maxPrefixLength; length >= 1; length -= 1) {
    const prefix = digits.slice(0, length);
    if (KNOWN_COUNTRY_CALLING_CODES.has(prefix)) {
      return true;
    }
  }
